...
class ValidAnagramSolutionTests extends AnyFunSpec {
  describe("Valid Anagram Solution tests") {
    it("Should return true when params are s = \"anagram\", t = \"nagaram\"") {
      ...
      assert(Solution.isAnagramHashTableArray("anagram", "nagaram") == true)
      assert(Solution.isAnagramHashTableArray2("anagram", "nagaram") == true)
    }
    ...
    it("Should return false when params are s = \"rat\", t = \"car\"") {
      ...
      assert(Solution.isAnagramHashTableArray("rat", "car") == false)
      assert(Solution.isAnagramHashTableArray2("rat", "car") == false)
    }
  }
}
